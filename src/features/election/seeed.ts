...
const seedSchoolsElection = async () => {
  ...
  // Create election votes (Andrea votes Public, Beatrice votes Private, Charles votes Public)
  const electionVotes = [
  ...
  // Create public preferences
  const publicPreferences = [
    // Andrea's voters: 5 prefer Public, 5 prefer Private
    ...
    // Beatrice's voters: All 20 prefer Private
    ...bengtVoters.map((voter) => ({
      voter_id: voter.id,
      election_id: electionId,
      alternative_id: privateId,
    ...
    // Charles's voters: 10 prefer Public, 5 prefer Private
